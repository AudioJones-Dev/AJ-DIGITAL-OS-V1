...
argument-hint: "[batch-id-commit-or-diff]"
---
...
- Root `AGENTS.md`, `CLAUDE.md`, and applicable validation requirements.
...
3. Run required lint, typecheck, targeted tests, full tests, and build commands where available and policy requires them.
4. Verify dependency manifests and lockfiles are internally consistent if dependencies changed.
...
- Do not excuse failed validation because deleted code looked unused.
- Do not claim production safety from local checks alone.
- If repair is required, return to planning/apply with a newly approved scope.
